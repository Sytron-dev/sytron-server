...
func SignUp() types.HandlerFunc {
	return func(c *fiber.Ctx) error {
		...
		if user.Token, user.RefreshToken, err = util.GenerateAllTokens(
			user.Email,
			user.ID.String(),
...
func Login() types.HandlerFunc {
	return func(c *fiber.Ctx) error {
		...
		token, refreshToken, _ := util.GenerateAllTokens(
			foundUser.Email,
			foundUser.ID.String(),
			constants.USER_ROLE_CONSUMER,
		)

		util.UpdateAllTokens(token, refreshToken, foundUser.ID.String())
